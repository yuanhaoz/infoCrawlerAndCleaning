...
/**
 * zhengyuanhao  2016/3/14
 * 简单实现：quora  
 * 实现功能：1.解析本地的“问题网页”和“作者网页”；
 *        2.解析函数的 对象 是 解析指定路径的 html文件得到的doc对象；
 *        3.分别实现了对问题页面的问题信息和答案信息的抽取，对作者页面的个人信息，粉丝数等信息的抽取。
 * "特征抽取"			      
...
//				System.out.println("问题内容为：" + name_s);
	...
	public static String questionExpandInfo(Document doc) {
		...
		if(as.size() != 0){
			Element a = as.get(0);
			Elements expandInfo = a.select("div.header")
									.select("div.question_details")
									.select("div.expanded_q_text");
...
//				System.out.println("问题附加信息为：" + expandInfo_s);
	...
	public static int questionLength(Document doc) {
		String content = User.questionContent(doc);
		String expandInfo = User.questionExpandInfo(doc);
		// 使用split得到文本的单词数目
		String[] question = content.split(" ");
		String[] questionExpand = expandInfo.split(" ");
		int length = question.length + questionExpand.length;
...
//			System.out.println("不存在Follow选项...");
	...
	public static String questionComment(Document doc) {
		...
		if(questions.size() !=0 ){
			Element question = questions.get(1);// 得到第二个action_item标签，即为Comment
	 ...
	 * @param doc,n
	 */
	public static String questionShare(Document doc) {
		Elements questions = doc.select("div.header").select("div.action_item");
		if(questions.size() !=0 ){
			Element question = questions.get(2);// 得到第三个action_item标签，即为Share
			Elements share = question.select("a[href=#]");
			if (share.size() == 0) {
				System.out.println("问题不存在分享栏目...");
				return "0";
			} else {
				Elements b = share.select("span[class=count]");
				if (b.size() == 0) {
					System.out.println("问题不存在分享！！！");
					return "0";
				} else {
					String share_number = b.text();
					System.out.println("问题分享数目为：" + share_number);
					return share_number;
				}
			}
		} else {
			System.out.println("不存在分享选项...");
			return "0";
		}
	}
...
//		System.out.println("第" + (n+1) + "条回答内容为：");
...
//		System.out.println(answer_context_s);
	 ...
	 * @param doc,n
	 */
	public static int answerLength(Document doc, int n) {
		String answer_context_s = User.answerContent(doc, n);
		// 使用split得到文本的单词数目
...
//		System.out.println("回答单词长度为：" + content_length);
	 ...
	 * @param doc,n
	 */
	public static String answerUpvote(Document doc, int n) {
...
//			System.out.println("支持票数为：" + answer_voters_s);
	...
	public static String answerComment(Document doc, int n) {
		...
		Element per_answer = comment_numbers.get(2*n);// 第奇数个action_item为Comment栏目
...
//				System.out.println("答案" + n + "评论为0。。。");
				...
				String comment_number = b.text();
				if(comment_number.contains("+")){
					comment_number = comment_number.substring(0, comment_number.length()-1);
				}
//				System.out.println("答案" + n + "的评论数目为：" + comment_number);
	 ...
	 * @param doc,n
	 */
	public static String answerShare(Document doc, int n) {
		Elements share_numbers = doc.select("div.pagedlist_item")
				.select("div[class^=action_bar]").select("div[class^=action_it]");
		if(share_numbers.size()!=0){
		Element per_answer = share_numbers.get(2*n+1);// 第偶数个action_item为Share栏目
		Elements share = per_answer.select("div[id]").select("a[class^=AnswerQuickShare]");
		if (share.size() == 0) {
//			System.out.println("答案" + n + "不存在Share选项！！！");
			return "0";
		} else {
			Elements b = share.select("span[class=count]");
			if (b.size() == 0) {
//				System.out.println("答案" + n + "分享为0...");
				return "0";
			} else {
				String share_number = b.text();
				if(share_number.contains("+")){
					share_number = share_number.substring(0, share_number.length()-1);
				}
//				System.out.println("答案" + n + "的分享数目为：" + share_number);
				return share_number;
			}
		}
		}else{
	 ...
	 * @param doc,n
	 */
	public static String answerURL(Document doc, int n) {
		Element content_i = doc.select("div.pagedlist_item")
				.select("div[class^=Answer Answer]").get(n);
		String content = content_i.attr("id");
		Elements title = doc.select("div.header").select("div.question_text_edit");
		...
		if (a.size() == 0) {
//			System.out.println("第" + (n+1) + "条回答不包含链接");
			return "0";
		} else {
			String urls = ""; // 用于保存链接
...
//			System.out.println("第" + (n+1) + "条回答包含链接数目为：" + a.size());
//			String exist = "1";
			String exist = a.size() + "";
			return exist;
		}
	...
	public static String authorName(Document doc, int n) {
		...
		// 用户存在Quora User匿名Anonymous的情况
		Elements c = a.select("a.user");
		String author_name_s = null;
		if (c.size() == 0) {
			author_name_s = "Anonymous";// 匿名用户
		} else {
	 ...
	 * @param doc,n
	 */
	public static String authorAnswer(Document doc, int n) {
...
//			System.out.println("没有得到作者的答案总数信息，建议重新爬取作者页面...");
			...
			// 去除3,986中的英文逗号
			if(author_Answers_s.contains(",")){
				author_Answers_s = author_Answers_s.replace(",","");
			}
//			System.out.println("第" + (n + 1) + "位作者 回答 问题的数目为：" + author_Answers_s);
	 ...
	 * @param doc,n
	 */
	public static String authorQuestion(Document doc, int n) {
...
//			System.out.println("没有得到作者的答案总数信息，建议重新爬取作者页面...");
			...
			if(authorQuestion.size() != 1){
				String authorQuestionCount = authorQuestion.get(0).text();
				// 去除3,986中的英文逗号
				if(authorQuestionCount.contains(",")){
					authorQuestionCount = authorQuestionCount.replace(",","");
				}
...
//				System.out.println("第" + (n + 1) + "位作者 提出 问题的数目--->难以获得");
	 ...
	 * @param doc,n
	 */
	public static String authorFollower(Document doc, int n) {
...
//			System.out.println("没有得到作者粉丝信息，建议重新爬取作者页面......");
			...
			// 去除3,986中的英文逗号
			if(author_followers_s.contains(",")){
				author_followers_s = author_followers_s.replace(",","");
			}
...
//			System.out.println("没有得到作者粉丝信息，建议重新爬取作者页面......");
			...
			if(author_following.size() != 1){
				...
				// 去除3,986中的英文逗号
				if(author_following_s.contains(",")){
					author_following_s = author_following_s.replace(",","");
				}
...
//				System.out.println("第" + (n + 1) + "位作者following数目--->难以获得");
	...
	public static int authorAminusQ(Document doc, int n){
		String answer = User.authorAnswer(doc, n);
		String question = User.authorQuestion(doc, n);
		int AminusQ = Integer.parseInt(answer) - Integer.parseInt(question);
//		System.out.println("AminusQ is: " + AminusQ);
		return AminusQ;
	}
