...
public class InformationExtraction2ExcelForHttpClient{
...
//		extraction("Data_mining");
		extraction("Data_structure_excel112");
	}
...
//				System.out.println("开始解析： " + path);
				...
				//得到主题网页爬取时间
				String subjectpagecrawlertime = FeatureExtractionForHttpClient.getCrawlerTime(path);
...
//				ArrayList<String> infolist = new ArrayList<String>();
				...
				titlelist.add("");
				titlelist.add("");
				titlelist.add(FeatureExtractionForHttpClient.questionContentWordLength(doc) + "");
				titlelist.add("1");
				titlelist.add(i + "");
				for(int j = 0; j < 13; j++){
				...
				for (int m = number; m < number + realanswernumber; m++) {
					...
					String answercontent = FeatureExtractionForHttpClient.answerContent(doc, m - number);
					int contentlength = FeatureExtractionForHttpClient.answerContentWordLength(doc, m - number);// 单词长度
					String upvote = "";              // 支持票数 
					String url = FeatureExtractionForHttpClient.answerURLs(doc, m - number);// 链接有无
					String comment = "";// 评论数量
					String authorName = FeatureExtractionForHttpClient.authorName(doc, m - number);            // 作者简历
...
//						System.out.println(authorpath + "   不存在");
						...
						follower = FeatureExtractionForHttpClient.authorFollowers(authorDoc, m - number);   // 作者粉丝
						know = FeatureExtractionForHttpClient.authorKnowAbout(authorDoc, m - number);       // 作者领域
						answer = FeatureExtractionForHttpClient.authorAnswers(authorDoc, m - number);       // 回答总数
						following = FeatureExtractionForHttpClient.authorFollowing(authorDoc, m - number);  // 作者偶像
						questions = FeatureExtractionForHttpClient.authorQuestions(authorDoc, m - number);  // 提问总数
					}
