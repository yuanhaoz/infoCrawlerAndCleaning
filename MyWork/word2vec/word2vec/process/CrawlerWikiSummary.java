...
public class CrawlerWikiSummary {
	
	private static String catalog2 = "F:\\test\\kt";

	public static void main(String[] args) {
		getContentAll(catalog2);
	}
	
	/**
	 * 得到所有关键词内容
	 */
	public static void getContentAll(String path){
		ArrayList<String> cata = DirFile.getFileNamesFromDirectorybyArraylist(catalog2);
		System.out.println(cata.size());
		for(int i = 0; i < cata.size(); i++){
			String keyword = cata.get(i);
//			System.out.println("keyword is: " + keyword);
			String filePath = catalog2 + "\\" + keyword + ".html";
...
//			getContent(doc, keyword);// 按标签存储内容
	...
	public static void getContent_all(Document doc, String keyword) {
		...
		if (body.size() != 0) {
			...
			try {
				CrawlerWikiSummary.writeFile(keyword, keyword + "_all", all);
			} catch (Exception e1) {
				e1.printStackTrace();
...
//							System.out.println("*******************************");
						...
						// 将该小部分内容写到文件中
						try {
							CrawlerWikiSummary.writeFile(keyword, title, content);
						} catch (Exception e1) {
...
//				list1.offer(e); // offer()加入元素
			...
			for (int i = 0; i < list.size(); i++) {
				Element child = list.get(i); // 取出并删除
				Elements toc = child.select("table#toc");
				if(toc.size()!=0){
					tocId = i;
...
//			System.out.println(all);
			try {
				CrawlerWikiSummary.writeFile(keyword, keyword + "_summary", all);
			} catch (Exception e1) {
				e1.printStackTrace();
	...
	public static void writeFile(String keyword, String file, String content) throws Exception {
		// 创建存储目录
		new File(catalog2 + "\\" + keyword).mkdir();
		// 创建待写文件
		// 判断是否存在 /\<>*?|:" 这些特殊符号
...
//			System.out.println("文件名规范...");
		...
		String filepath = catalog2 + "\\" + keyword + "\\" + file + ".txt";
