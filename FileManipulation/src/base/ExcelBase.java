...
/**
 *创建、读取和修改Excel表格（Excel表格不存在的情况下会自动创建），包括String、Integer、Double类型
 * @author MJ
 * @description
 * @usage 1)创建类ExcelTest继承ExcelBase；<br/>
 *2)定义Excel文件名xlsFileName，在函数run()里面写eb.go(xlsFileName)；<br/>
 *3)将对Excel的操作写到函数process()里面；<br/>
 *4)在main函数里面ExcelTest et=new ExcelTest();  et.run(et).
 *
 */
public abstract class ExcelBase {
	...
	public void setStringValue(int Column, int Row, String value) {
		...
		if (sheetNumber <= 0)
			...
			wst = wwb.getSheet(0);
		try {
			wst.addCell(new Label(Column, Row, value));//int int String
	...
	/**
	 * 获取第sheetID个sheet、ColumnName列、第Row行对应格子的字符串
	...
	/**
	 * 设置第sheetID个sheet、ColumnName列、第Row行对应格子的内容为浮点数value
	 * @param sheetID sheet编号
	 * @param ColumnName 列名
	 * @param Row 行号
	 * @param value 数值
