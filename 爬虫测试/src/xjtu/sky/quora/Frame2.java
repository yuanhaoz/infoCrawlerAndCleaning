...
public class Frame2 extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == cross) {
			...
			String filepath = "F:\\WekaAnalysis\\" + course + "\\" + kw + ".arff";
			try {
				Instances data = xjtu.sky.weka.test.getFileInstances(filepath);
				Evaluation crossEvaluation = null, testEvaluation = null;
				System.out.println("===============交叉验证================");
				// 交叉验证 new NaiveBayes(); new IBk(); new SMO(); new J48();
				if (cb4.getState()) {
					nb = new NaiveBayes();
					crossEvaluation = xjtu.sky.weka.test.crossValidation(data, nb, 10);
					testEvaluation = xjtu.sky.weka.test.evaluateTestData(data, nb);
				} else if (cb5.getState()) {
					J48 j= new J48();
					crossEvaluation = xjtu.sky.weka.test.crossValidation(data, j, 10);
					testEvaluation = xjtu.sky.weka.test.evaluateTestData(data, j);
					
				} else if (cb6.getState()) {
					smo = new SMO();
					crossEvaluation = xjtu.sky.weka.test.crossValidation(data, smo, 10);
					testEvaluation = xjtu.sky.weka.test.evaluateTestData(data, smo);
				}
				
				xjtu.sky.weka.test.printEvalDetail(crossEvaluation);   //控制台显示
		        show = show + crossEvaluation.toClassDetailsString() + 
		        		crossEvaluation.toSummaryString() + 
		        		crossEvaluation.toMatrixString();       //resultPane显示
		        
				xjtu.sky.weka.test.printEvalDetail(testEvaluation);   //控制台显示
